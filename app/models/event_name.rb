  ...
  def payloads_per_hour
    ...
    (0..23).each do |hour|
      requested_ats.each do |requested_at|
        ...
        times["#{(Time.now - 60*60*(hour+1)).strftime("%l%p")+"-"+(Time.now - 60*60*(hour)).strftime("%l%p")}"] ||= 0
        if (Time.now - 60*60*hour) > payload_time  && (Time.now - 60*60*(hour+1)) < payload_time
            times["#{(Time.now - 60*60*(hour+1)).strftime("%l%p")+"-"+(Time.now - 60*60*(hour)).strftime("%l%p")}"] += 1
        end
      end
    end
    return times
